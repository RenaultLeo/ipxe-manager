...
Paramètres noyau ajoutés : **`vga=791 video=vesafb:ywrap,mtrr`** (désactiver : `PROXMOX_VGA_PARAMS=false`), **`ramdisk_size=16777216`**, **`rw quiet splash=silent`**, **`initrd=<nom fichier>`**. Config auto : à l’enregistrement ou lors du choix de la **config courante** sur la fiche ISO, **`answer.toml`** est injecté dans **`proxmox-netboot.iso`** avec **`auto-installer-mode.toml`** (`mode = "iso"`) via **xorriso** (tâche Celery, quelques secondes) ; le menu ajoute **`proxmox-start-auto-installer`**. Nécessite **`xorriso`** (`apt install xorriso`). NFS n’est pas supporté nativement par l’installateur Proxmox (contrairement à Ubuntu casper).

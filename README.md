...
Pour **Rocky Linux**, **AlmaLinux**, **CentOS** et **Fedora** (Anaconda), l’ISO est extraite **en entier** (arbre type DVD selon l’ISO : `BaseOS` / `AppStream`, `.treeinfo`, `images/`, etc.). Les menus ajoutent selon l’OS : **`inst.repo=`** (Rocky, Alma, CentOS) ou **`inst.stage2=`** + **`rd.neednet=1`** (Fedora Live extraite), plus **`ip=dhcp`** si besoin. Si le type d’OS **Fedora** (ou autre EL) a **« extraction complète »** coché dans Paramètres **sans** liste de noms de fichiers, le moteur intégré est utilisé automatiquement (plus d’échec sur « configuration vide »). **Fedora Workstation Live** : le noyau est souvent **`boot/x86_64/loader/linux`** (pas `vmlinuz`) ; l’install réseau « classique » décrite par [iPXE howto Fedora](https://ipxe.org/howto/fedora) utilise plutôt une arborescence **Everything** / **netinst** avec `images/pxeboot/vmlinuz`.

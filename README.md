...
Paramètres noyau ajoutés : **`vga=791 video=vesafb:ywrap,mtrr`** (désactiver : `PROXMOX_VGA_PARAMS=false`), **`ramdisk_size=16777216`**, **`rw quiet splash=silent`**, **`initrd=<nom fichier>`**. Config auto : à l’enregistrement ou à la régénération des menus, **`answer.toml`** est injecté dans **`proxmox-netboot.iso`** avec **`auto-installer-mode.toml`** (`mode = "iso"`) ; le menu ajoute **`proxmox-start-auto-installer`** (plus de `proxmox-installer.answer-file=`). Nécessite **`7z`** sur le serveur. NFS n’est pas supporté nativement par l’installateur Proxmox (contrairement à Ubuntu casper).

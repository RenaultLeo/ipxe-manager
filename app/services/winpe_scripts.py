...
$p = Start-Process -FilePath 'dism.exe' -ArgumentList $dismArgs -Wait -PassThru -NoNewWindow
if ($p.ExitCode -ne 0) {{
    Write-Host "DISM Add-Driver a signale des erreurs (code $($p.ExitCode)) — deploiement continue." -ForegroundColor Yellow
    Write-Host 'Certains paquets peuvent etre ignores (ex. viosock) ; verifiez dism.log si besoin.' -ForegroundColor Yellow
    exit 0
}}
Write-Host 'Pilotes injectes.' -ForegroundColor Green
exit 0
"""

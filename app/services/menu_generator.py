...
def regenerate_all(db: Session) -> list[str]:
    ...
    for os_type in os_types:
        ...
        entries = []
        for v in versions:
            be: BootEntry | None = v.boot_entry
            entries.append(
                {
                    "id": v.id,
        ...
        tmpl_name = "linux.ipxe.j2" if os_type.boot_type == "linux" else "windows.ipxe.j2"
        # Fall back to linux template if custom type has no dedicated template

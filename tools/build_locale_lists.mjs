...
const PAIRS_DE = [
  ...
  ["Other", "Andere"],
  ["Boot options (imgargs)", "Boot-Optionen (imgargs)"],
  ["ESXi artifacts (kernel + HTTP modules)", "ESXi-Artefakte (Kernel + HTTP-Module)"],
  ["Legacy bootloader (mboot.c32)", "Legacy-Bootloader (mboot.c32)"],
  ["UEFI bootloader (mboot.efi)", "UEFI-Bootloader (mboot.efi)"],
  ["iPXE modules (Legacy)", "iPXE-Module (Legacy)"],
  ["iPXE modules (UEFI)", "iPXE-Module (UEFI)"],
  [
    "iPXE preload order — Legacy menu profile.",
    "iPXE-Vorladereihenfolge — Legacy-Menüprofil.",
  ],
  [
    "iPXE preload order — UEFI menu profile.",
    "iPXE-Vorladereihenfolge — UEFI-Menüprofil.",
  ],
  ["ipxe-boot-legacy.cfg", "ipxe-boot-legacy.cfg"],
  ["ipxe-boot.cfg (UEFI)", "ipxe-boot.cfg (UEFI)"],
  [
  ...
  [
    "No scripts yet. Add one from this page or from the ISO release page after reviewing boot files.",
    "Noch keine Skripte. Fügen Sie hier oder auf der ISO-Versionsseite eines hinzu (nach Prüfung der Boot-Dateien oben).",
  ],
  ["Add script", "Skript hinzufügen"],
  [
    "Pick an ISO release, then upload a .ipxe file.",
    "ISO-Version wählen und eine .ipxe-Datei hochladen.",
  ],
  ["ISO release", "ISO-Version"],
  ["Editor", "Editor"],
  ["Name", "Name"],
  ...
  [
    ...
    "Sobald Sie oben die Boot-Dateien geprüft haben, laden Sie Ihr Skript (.ipxe oder .txt) hoch. Es erscheint im Untermenü Sonstiges oder unter Benutzerdefinierte Skripte in den iPXE-Menüs.",
  ],
  ["Upload", "Hochladen"],
...
const PAIRS_ES = [
  ...
  ["Other", "Otro"],
  ["Boot options (imgargs)", "Opciones de arranque (imgargs)"],
  ["ESXi artifacts (kernel + HTTP modules)", "Artefactos ESXi (kernel + módulos HTTP)"],
  ["Legacy bootloader (mboot.c32)", "Cargador Legacy (mboot.c32)"],
  ["UEFI bootloader (mboot.efi)", "Cargador UEFI (mboot.efi)"],
  ["iPXE modules (Legacy)", "Módulos iPXE (Legacy)"],
  ["iPXE modules (UEFI)", "Módulos iPXE (UEFI)"],
  [
    "iPXE preload order — Legacy menu profile.",
    "Orden de precarga iPXE — perfil de menú Legacy.",
  ],
  [
    "iPXE preload order — UEFI menu profile.",
    "Orden de precarga iPXE — perfil de menú UEFI.",
  ],
  ["ipxe-boot-legacy.cfg", "ipxe-boot-legacy.cfg"],
  ["ipxe-boot.cfg (UEFI)", "ipxe-boot.cfg (UEFI)"],
  [
  ...
  [
    "No scripts yet. Add one from this page or from the ISO release page after reviewing boot files.",
    "Sin scripts por ahora. Añada uno desde aquí o desde la ficha de la versión ISO (tras revisar los ficheros de arranque).",
  ],
  ["Add script", "Añadir script"],
  [
    "Pick an ISO release, then upload a .ipxe file.",
    "Elija una versión ISO y suba un archivo .ipxe.",
  ],
  ["ISO release", "Versión ISO"],
  ["Name", "Nombre"],
  [
    "UEFI via EFI drivers (virtio, e1000…)",
  ...
  [
    "Once you’ve checked your boot files above, upload your script (.ipxe or .txt). It appears under the Other submenu or Custom scripts in iPXE menus.",
    "Cuando haya revisado arriba los ficheros de arranque, suba su script (.ipxe o .txt). Aparecerá en el submenú Otro o en Scripts personalizados de los menús iPXE.",
  ],
  ["Upload", "Subir"],
  ["Script file", "Fichero de script"],
  ["Current script", "Script actual"],
  [
    "Uploading a new file replaces this one and regenerates menus.",
    "Al subir un fichero nuevo se sustituye éste y se regeneran los menús.",
  ],
...
const PAIRS_IT = [
  ...
  ["Other", "Altro"],
  ["Boot options (imgargs)", "Opzioni di boot (imgargs)"],
  ["ESXi artifacts (kernel + HTTP modules)", "Artefatti ESXi (kernel + moduli HTTP)"],
  ["Legacy bootloader (mboot.c32)", "Bootloader Legacy (mboot.c32)"],
  ["UEFI bootloader (mboot.efi)", "Bootloader UEFI (mboot.efi)"],
  ["iPXE modules (Legacy)", "Moduli iPXE (Legacy)"],
  ["iPXE modules (UEFI)", "Moduli iPXE (UEFI)"],
  [
    "iPXE preload order — Legacy menu profile.",
    "Ordine di precaricamento iPXE — profilo menu Legacy.",
  ],
  [
    "iPXE preload order — UEFI menu profile.",
    "Ordine di precaricamento iPXE — profilo menu UEFI.",
  ],
  ["ipxe-boot-legacy.cfg", "ipxe-boot-legacy.cfg"],
  ["ipxe-boot.cfg (UEFI)", "ipxe-boot.cfg (UEFI)"],
  [
  ...
  [
    "No scripts yet. Add one from this page or from the ISO release page after reviewing boot files.",
    "Ancora nessuno script. Aggiungerlo qui o dalla pagina della versione ISO (dopo aver controllato i file di avvio).",
  ],
  ["Add script", "Aggiungi script"],
  [
    "Pick an ISO release, then upload a .ipxe file.",
    "Scegliere una versione ISO e caricare un file .ipxe.",
  ],
  ["ISO release", "Versione ISO"],
  ["Editor", "Editor"],
  ["Name", "Nome"],
  ["iPXE firmware", "Firmware iPXE"],
  [
  ...
  [
    "Once you’ve checked your boot files above, upload your script (.ipxe or .txt). It appears under the Other submenu or Custom scripts in iPXE menus.",
    "Dopo aver verificato i file di avvio sopra, caricate il vostro script (.ipxe o .txt). Compare nel sottomenu Altro o in Script personalizzati dei menu iPXE.",
  ],
  ["Upload", "Carica"],
  ["Script file", "File script"],
  ["Current script", "Script corrente"],
  [
    "Uploading a new file replaces this one and regenerates menus.",
    "Caricare un nuovo file sostituisce questo e rigenera i menu.",
  ],
...
const PAIRS_PT = [
  ...
  ["Other", "Outro"],
  ["Boot options (imgargs)", "Opções de boot (imgargs)"],
  ["ESXi artifacts (kernel + HTTP modules)", "Artefatos ESXi (kernel + módulos HTTP)"],
  ["Legacy bootloader (mboot.c32)", "Carregador Legacy (mboot.c32)"],
  ["UEFI bootloader (mboot.efi)", "Carregador UEFI (mboot.efi)"],
  ["iPXE modules (Legacy)", "Módulos iPXE (Legacy)"],
  ["iPXE modules (UEFI)", "Módulos iPXE (UEFI)"],
  [
    "iPXE preload order — Legacy menu profile.",
    "Ordem de pré-carregamento iPXE — perfil de menu Legacy.",
  ],
  [
    "iPXE preload order — UEFI menu profile.",
    "Ordem de pré-carregamento iPXE — perfil de menu UEFI.",
  ],
  ["ipxe-boot-legacy.cfg", "ipxe-boot-legacy.cfg"],
  ["ipxe-boot.cfg (UEFI)", "ipxe-boot.cfg (UEFI)"],
  [
  ...
  [
    "No scripts yet. Add one from this page or from the ISO release page after reviewing boot files.",
    "Ainda sem scripts. Adicione um aqui ou na página da versão ISO (após rever os ficheiros de arranque).",
  ],
  ["Add script", "Adicionar script"],
  [
    "Pick an ISO release, then upload a .ipxe file.",
    "Escolha uma versão ISO e carregue um ficheiro .ipxe.",
  ],
  ["ISO release", "Versão ISO"],
  ["Editor", "Editor"],
  ["Name", "Nome"],
  ["iPXE firmware", "Firmware iPXE"],
  [
  ...
  [
    "Once you’ve checked your boot files above, upload your script (.ipxe or .txt). It appears under the Other submenu or Custom scripts in iPXE menus.",
    "Depois de verificar os ficheiros de arranque acima, carregue o seu script (.ipxe ou .txt). Aparece no sub-menu Outro ou em Scripts personalizados dos menus iPXE.",
  ],
  ["Upload", "Carregar"],
  ...
  [
    "Uploading a new file replaces this one and regenerates menus.",
    "Ao carregar um ficheiro novo substitui este e regenera os menus.",
  ],
